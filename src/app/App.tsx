...
export default function App() {
  ...
  return (
    <div
      className={`theme-${theme} min-h-screen p-2 sm:p-4 md:p-8 transition-colors duration-300 app-bg`}
    >
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div
          className="rounded-lg shadow-lg p-4 sm:p-6 mb-4 sm:mb-6 transition-colors duration-300 relative card"
        >
          ...
          <div className="absolute top-3 right-3 sm:top-4 sm:right-4 flex items-center gap-2">
            ...
            <div className="relative">
              <select
                ...
                title="Select theme"
                className="appearance-none px-2 py-1.5 pr-7 text-xs sm:text-sm rounded-lg focus:outline-none focus:ring-2 transition-all duration-300 cursor-pointer themed-select"
              >
                {themes.map(t => (
              ...
              <Palette
                className="absolute right-2 top-1/2 -translate-y-1/2 w-3 h-3 pointer-events-none palette-icon"
              />
            </div>
          </div>

          <h1
            className="text-2xl sm:text-3xl mb-4 sm:mb-6 text-center transition-colors duration-300 pr-16 text-accent"
          >
          ...
          <div className="grid sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
              <label
                className="flex items-center gap-2 mb-2 text-sm sm:text-base transition-colors duration-300 text-themed"
              >
              ...
              <input
                ...
                maxLength={50}
                className="w-full px-3 sm:px-4 py-2 text-sm sm:text-base rounded-lg focus:outline-none focus:ring-2 transition-all duration-300 input-themed"
              />
            </div>
            <div>
              <label
        ...
        <div
          className="rounded-lg shadow-lg p-3 sm:p-6 mb-4 sm:mb-6 transition-colors duration-300 card"
        >
          <h2
            className="text-lg sm:text-xl mb-3 sm:mb-4 transition-colors duration-300 text-themed"
          >
            Daily Habit Tracker
          </h2>
          <div className="overflow-x-auto -mx-3 sm:mx-0 px-3 sm:px-0">
            <table className="w-full border-collapse min-w-max">
              <thead>
                <tr>
                  <th
                    className="p-1.5 sm:p-2 text-left text-xs sm:text-sm min-w-[120px] sm:min-w-[150px] sticky left-0 z-10 transition-colors duration-300 table-header-cell"
                  >
                    Habit
                  </th>
                  {Array.from({ length: daysInMonth }, (_, i) => {
                    ...
                    return (
                      <th
                        key={i}
                        className={`p-1 sm:p-2 text-center text-xs sm:text-sm w-8 sm:w-10 transition-colors duration-300 ${isToday ? 'table-header-today' : 'table-header-normal'}`}
                      >
                        {day}
                      </th>
                    );
              ...
              <tbody>
                {data.habits.map((habit, habitIndex) => {
                  ...
                  return (
                    <tr key={habit.id}>
                      <td
                        className="p-1.5 sm:p-2 sticky left-0 z-10 transition-colors duration-300 td-sticky"
                      >
                        <input
                          type="text"
                          value={habit.name}
                          ...
                          maxLength={30}
                          className="w-full px-1.5 sm:px-2 py-1 text-xs sm:text-sm rounded focus:outline-none focus:ring-1 transition-all duration-300 input-themed-light"
                        />
                        {/* Streak & Rate badges below habit name */}
                        {habit.name.trim() && (
                          <div className="flex items-center gap-2 mt-1 px-0.5">
                            <span
                              className={`text-[10px] sm:text-xs font-medium flex items-center gap-0.5 ${currentStreak >= 7 ? 'streak-success' : currentStreak >= 3 ? 'streak-warning' : 'streak-default'}`}
                            ...
                            <span
                              className={`text-[10px] sm:text-xs font-medium ${completionRate >= 70 ? 'streak-success' : completionRate >= 40 ? 'streak-warning' : 'streak-default'}`}
                            >
                              {completionRate}%
                            </span>
                          </div>
                      ...
                      {Array.from({ length: daysInMonth }, (_, dayIndex) => {
                        ...
                        return (
                          <td
                            key={day}
                            className={`p-1 sm:p-2 text-center transition-colors duration-300 ${isToday ? 'td-today' : 'td-normal'}`}
                          >
                            <input
                              type="checkbox"
                              checked={isCompleted}
                              ...
                              aria-label={`${habit.name || `Habit ${habitIndex + 1}`} - Day ${day}`}
                              className="w-3.5 h-3.5 sm:w-4 sm:h-4 rounded cursor-pointer disabled:cursor-not-allowed disabled:opacity-30 transition-all duration-300 checkbox-themed"
                            />
                          </td>
                        );
                      })}
              ...
              {Array.from({ length: daysInMonth }, (_, i) => {
                ...
                return (
                  <div key={day} className="flex flex-col items-center min-w-[44px] sm:min-w-0">
                    <div
                      className="text-[10px] sm:text-xs mb-1 transition-colors duration-300 text-secondary-themed"
                    >
                      D{day}
                    </div>
                    <div
                      className={`w-full h-10 sm:h-12 rounded flex items-center justify-center text-xs sm:text-sm font-medium transition-all duration-300 ${percentage >= 70 ? 'score-success' : percentage >= 40 ? 'score-warning' : 'score-danger'} ${score === 0 ? 'opacity-30' : ''}`}
                    >
                      {score}/{maxScore}
                    </div>
                  </div>
        ...
        >
          <h2
            className="flex items-center gap-2 text-lg sm:text-xl mb-3 sm:mb-4 transition-colors duration-300 text-themed"
          >
        ...
        <div
          className="rounded-lg shadow-lg p-6 sm:p-8 text-center transition-all duration-300 quote-gradient"
        >
          <p className="text-lg sm:text-xl md:text-2xl text-white italic leading-relaxed">
            "{quote.text}"
          </p>
          <p className="mt-2 text-sm sm:text-base opacity-90 quote-author">
            — {quote.author}
          </p>
        </div>
      </div>
